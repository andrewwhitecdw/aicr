...
| [`aicr-triage`](https://github.com/NVIDIA/aicr/blob/main/.agents/skills/aicr-triage/SKILL.md) | Triage the NVIDIA AICR project board (project 248): classify new issues, promote P2→P1, demote Ready→Backlog, surface closures — with per-bucket confirmation before applying. |
...
- **Explicit.** A contributor can name a skill directly to force its use. The syntax differs by agent: Claude Code uses `/skill-name` (e.g. `/aicr-triage`); Codex uses `$skill-name` (e.g. `$aicr-triage`).
